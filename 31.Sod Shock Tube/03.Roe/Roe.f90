...
!!!    This program solves Riemann problem for the Euler equations using Roe's first-order upwind methods
...
103    format('zone',1x'i=',1x,i5,2x,'f=point')
       ...
       u1 = ru1/rho1
       u4 = ru4/rho4
       p1 = (gamma-1.0)*(ret1-0.5*ru1*ru1/rho1)
       p4 = (gamma-1.0)*(ret4-0.5*ru4*ru4/rho4)
       h1 = (ret1+p1)/rho1
       h4 = (ret4+p4)/rho4
       ...
       rr = SQRT(rho1/rho4)
       rhoavg = rr*rho4
       uavg = (u4+u1*rr)/(1.0+rr)
       havg = (h4+h1*rr)/(1.0+rr)
       aavg = SQRT((gamma-1.0)*(havg-0.5*uavg*uavg))
...
!!!!!!!!!!!!!!!!!!!!!!!!!Alternative and equivalent flux formulae!!!!!!!!!!!!!!!!!!!!!!!!!
       f1 = ru4&
    &        +dv1*MIN(0.0,lambda1)&
    &        +dv2*MIN(0.0,lambda2)*0.5*rhoavg/aavg&
    &        +dv3*ABS(MIN(0.0,lambda3))*0.5*rhoavg/aavg
       f2 = ru4*u4+p4&
    &        +dv1*MIN(0.0,lambda1)*uavg&
    &        +dv2*MIN(0.0,lambda2)*(uavg+aavg)*0.5*rhoavg/aavg&
    &        +dv3*ABS(MIN(0.0,lambda3))*(uavg-aavg)*0.5*rhoavg/aavg
       f3 = ret4*u4+p4*u4&
    &        +dv1*MIN(0.0,lambda1)*0.5*uavg*uavg&
    &        +dv2*MIN(0.0,lambda2)*(havg+aavg*uavg)*0.5*rhoavg/aavg&
    &        +dv3*ABS(MIN(0.0,lambda3))*(havg-aavg*uavg)*0.5*rhoavg/aavg


!!!!!!!!!!!!!!!!!!!!!!!!!Alternative and equivalent flux formulae!!!!!!!!!!!!!!!!!!!!!!!!!
!       f1 = ru1&
!    &        -dv1*MAX(0.0,lambda1)&
!    &        -dv2*MAX(0.0,lambda2)*0.5*rhoavg/aavg&
!    &        -dv3*ABS(MAX(0.0,lambda3))*0.5*rhoavg/aavg
!       f2 = ru1*u1+p1&
!    &        -dv1*MAX(0.0,lambda1)*uavg&
!    &        -dv2*MAX(0.0,lambda2)*(uavg+aavg)*0.5*rhoavg/aavg&
!    &        -dv3*ABS(MAX(0.0,lambda3))*(uavg-aavg)*0.5*rhoavg/aavg
!       f3 = ret1*u1+p1*u1&
!    &        -dv1*MAX(0.0,lambda1)*0.5*uavg*uavg&
!    &        -dv2*MAX(0.0,lambda2)*(havg+aavg*uavg)*0.5*rhoavg/aavg&
!    &        -dv3*ABS(MAX(0.0,lambda3))*(havg-aavg*uavg)*0.5*rhoavg/aavg
